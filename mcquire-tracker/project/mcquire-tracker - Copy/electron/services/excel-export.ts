...
export async function generateFullTrackerExport(outputPath: string): Promise<void> {
  const db = getDb()
  const wb = new ExcelJS.Workbook()
  wb.creator = 'McQuire Tracker'
...
export function validateExpenseReportReadiness(dateFrom: string, dateTo: string): { valid: boolean; blocking: string[]; warnings: string[] } {
  const db = getDb()
