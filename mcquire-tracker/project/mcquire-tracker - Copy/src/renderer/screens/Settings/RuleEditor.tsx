...
export default function RuleEditor() {
  ...
  const load = useCallback(() => {
    ...
    window.api.listTripDates().then(setTripDates)
  }, [])
  useEffect(() => { load() }, [load])
  ...
  const handleSave = async () => {
    ...
    const result = isNew
      ? await window.api.createRule(payload)
      : await window.api.updateRule(editing.id, payload)
    setSaving(false)
    setEditing(null)
    load()
    if (result?.resolved > 0) alert(`✅ Rule saved. ${result.resolved} previously pending transactions were auto-classified by this rule.`)
  }
  ...
  const handleTest = async () => {
    const r = await window.api.testRuleMatch(editing)
    setTestResult(r)
  }

  const addTrip = async () => {
    await window.api.addTripDate(newTrip)
    setNewTrip({ trip_name: "", date_from: "", date_to: "" })
    load()
  }

  const deleteTrip = async (id: string) => {
    await window.api.deleteTripDate(id)
    load()
  }
        ...
        <div className="space-y-2 mb-3">
          {tripDates.map(t => (
            <div key={t.id} className="flex items-center justify-between text-sm bg-slate-50 rounded px-3 py-2">
              ...
              <span className="text-gray-500">{t.date_from} → {t.date_to}</span>
              ...
              <div className="border-t pt-4">
                ...
                {testResult && (
                  <div className="text-sm">
                    <div className="font-semibold text-gray-700 mb-1">This rule would match <span className="text-brand-blue">{testResult.matches}</span> existing transactions</div>
