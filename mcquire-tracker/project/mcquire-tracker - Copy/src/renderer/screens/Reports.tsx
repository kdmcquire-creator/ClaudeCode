...
export default function Reports() {
  ...
  const checkReadiness = async () => {
    ...
    setError(null)
    try {
      const result = await window.api.reports.checkExpenseReportReadiness().catch(() => null)
      if (result) {
        setReadiness({
          ready: !result.blockers?.length,
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (result?.filePath || result?.path || result?.file) {
        const path = result.filePath ?? result.path ?? result.file
        setLastGenerated({ path, report: REPORT_TYPES.find(r => r.id === selectedReport)?.label ?? selectedReport })
      } else if (result) {
        setLastGenerated({ path: "", report: REPORT_TYPES.find(r => r.id === selectedReport)?.label ?? selectedReport })
      } else {
